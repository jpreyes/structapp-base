...
def _get_project_inspection_raw(inspection_id: str):
    """
    Obtiene una inspección sin firmar las URLs de fotos.
    Usar internamente para operaciones de escritura.
    """
    result = (
        supa()
        .table("project_inspections")
...
def get_project_inspection(inspection_id: str):
    data = _get_project_inspection_raw(inspection_id)
...
def append_inspection_photo(inspection_id: str, photo: dict) -> Tuple[dict, dict]:
    inspection = _get_project_inspection_raw(inspection_id)
    if not inspection:
        raise ValueError("Inspección no encontrada")
    photos = inspection.get("photos") or []
    # Asegurar que guardamos storage_path en lugar de URL firmada
    photo_to_save = {**photo}
    if photo_to_save.get("storage_path"):
        # Usar storage_path como URL para almacenar (no la URL firmada)
        photo_to_save["url"] = photo_to_save["storage_path"]
    photos.append(photo_to_save)
...
def update_inspection_photo_comment(inspection_id: str, photo_id: str, comment: str | None) -> dict | None:
    ...
    for item in photos:
        if item.get("id") == photo_id or (not item.get("id") and item.get("url") == photo_id):
            ...
            break
    supa().table("project_inspections").update({"photos": photos}).eq("id", inspection_id).execute()
    # Retornar con URL firmada para la respuesta
    if updated:
        path = updated.get("storage_path") or updated.get("url")
        if path:
            updated["url"] = sign_storage_url(path)
    return updated
...
def get_project_inspection_damage_photo(photo_id: str):
    result = (
        supa()
        .table("project_inspection_damage_photos")
        .select("*")
...
def delete_project_inspection_damage_photo(photo_id: str):
    from services.media_service import delete_stored_inspection_photo

    photo = get_project_inspection_damage_photo(photo_id)
    if photo and photo.get("photo_url"):
        delete_stored_inspection_photo(photo["photo_url"])
