...
const TaskGantt = ({ tasks, onSelectTask }: TaskGanttProps) => {
  if (!tasks.length) {
    return (
      <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
        No hay tareas registradas para construir el diagrama de Gantt.
      </Typography>
    );
  }

  const sorted = [...tasks].sort((a, b) => dayjs(a.start_date).diff(dayjs(b.start_date)));
  const minDate = sorted.reduce(
    (acc, task) => (dayjs(task.start_date).isBefore(acc) ? dayjs(task.start_date) : acc),
    dayjs(sorted[0].start_date)
  );
  const maxDate = sorted.reduce(
    (acc, task) => (dayjs(task.end_date).isAfter(acc) ? dayjs(task.end_date) : acc),
    dayjs(sorted[0].end_date)
  );
  const totalDays = Math.max(maxDate.diff(minDate, "day") + 1, 1);
  const dayLabels = Array.from({ length: totalDays }).map((_, index) =>
    minDate.add(index, "day").format("DD/MM")
  );
    ...
    <Box sx={{ mt: 3, overflowX: "auto" }}>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, mt: 1 }}>
        {sorted.map((task) => {
          ...
          const paletteColor =
            statusMeta.color === "default" ? "grey.600" : `${statusMeta.color}.main`;
          const borderColor =
            statusMeta.color === "default" ? "grey.500" : `${statusMeta.color}.main`;
          return (
            <Box
              key={task.id}
              sx={{
              ...
              {Array.from({ length: totalDays }).map((_, index) => {
                ...
                return (
                  <Box
                    ...
                    className={isBar ? "gantt-bar" : undefined}
                  />
                );
              })}
            </Box>
