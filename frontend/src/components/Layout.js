...
import { AppBar, Box, Breadcrumbs, Button, Divider, Drawer, IconButton, List, ListItemButton, ListItemIcon, ListItemText, Stack, Switch, Toolbar, Typography, } from "@mui/material";
...
const Layout = () => {
    ...
    const navItems = useMemo(() => [
        ...
        { label: "Configuración", icon: _jsx(SettingsIcon, {}), path: "/settings", requiresAuth: true },
    ...
    const drawerContent = (_jsxs(Box, { sx: { display: "flex", flexDirection: "column", height: "100%", p: 2, gap: 2 }, children: [_jsxs(Box, { sx: {
                    ...
                    background: theme.palette.mode === "light" ? "#ffffff" : theme.palette.background.paper,
                            ...
                            color: "#fff",
                        }, children: _jsx(ArchitectureIcon, { fontSize: "small" }) }), _jsxs(Box, { children: [_jsx(Typography, { variant: "h6", fontWeight: 700, children: "StructApp" }), _jsx(Typography, { variant: "body2", color: "text.secondary", children: "Gesti\u00F3n estructural" })] })] }), _jsx(Divider, {}), _jsx(List, { disablePadding: true, children: navItems
                    .filter((item) => (item.showWhenLoggedOut ? !token : true))
                    .map((item) => {
                    if (isSectionItem(item)) {
                        return (_jsx(ListItemText, { primary: _jsx(Typography, { variant: "overline", color: "text.secondary", sx: {
                                    pl: 2,
                                    pt: 2,
                                    pb: 0.5,
                                    display: "block",
                    ...
                    return (_jsxs(ListItemButton, { selected: selected, disabled: item.requiresAuth && !token, onClick: () => {
                ...
                }) }), _jsx(Box, { sx: { flexGrow: 1 } }), _jsx(Divider, {}), _jsx(Box, { sx: { p: 1 }, children: _jsxs(Stack, { spacing: 1.5, children: [_jsxs(Stack, { direction: "row", alignItems: "center", gap: 1, children: [_jsx(Switch, { color: "primary", onChange: () => useThemeStore.getState().toggle(), checked: themeMode === "dark", inputProps: { "aria-label": "Cambiar tema" } }), _jsxs(Box, { children: [_jsxs(Typography, { variant: "body2", fontWeight: 700, children: ["Tema ", themeMode === "light" ? "claro" : "oscuro"] }), _jsx(Typography, { variant: "caption", color: "text.secondary", children: "Ajusta el contraste seg\u00FAn tu entorno." })] })] }), _jsx(Button, { variant: token ? "outlined" : "contained", color: "primary", fullWidth: true, onClick: () => {
                                if (token) {
                                    setToken(null);
                                    setUser(undefined);
                                    navigate("/login");
                            ...
                            }, children: token ? "Cerrar sesión" : "Iniciar sesión" })] }) })] }));
    return (_jsxs(Box, { sx: { display: "flex", background: theme.palette.background.default }, children: [_jsx(AppBar, { position: "fixed", sx: {
                    ...
                    ml: { sm: `${drawerWidth}px` },
                                    ...
                                    }, children: token ? "Cerrar sesión" : "Iniciar sesión" })] })] }) }), _jsxs(Box, { component: "nav", sx: { width: { sm: drawerWidth }, flexShrink: { sm: 0 } }, children: [_jsx(Drawer, { variant: "temporary", open: mobileOpen, onClose: () => setMobileOpen(false), ModalProps: { keepMounted: true }, sx: {
